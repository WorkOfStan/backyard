...
function socialLoginPseudoConstructor($internalId, $userLanguage){
    if(!($internalId && $userLanguage)){
        ...
        if(!$internalId)$internalId=$tempResult['user_id'];//pokud nebylo správně hack_owner_id, tak $ownerId z databáze dle social login
        if(!$userLanguage)$userLanguage=$tempResult['user_language'];//_REQUEST['owner_language'] overrides what is set in the database    
    } //else keep $ownerId && $ownerLanguage previously defined    
...
function getInternalUserId(){
    ...
    my_error_log("authId: ".  DumpArrayAsOneLine($authId),5);
    if($authId){
