...
$(document).on(
        "focusout", 'textarea.ajaxsave, input[type="text"].ajaxsave, input[type="date"].ajaxsave, input[type="email"].ajaxsave, input[type="tel"].ajaxsave', function(e) //keyup píše každé písmeno
        {
            if ($(this).attr("placeholder") !== $(this).val()) {
                ...
                tempObject = new Object;
                if (projectId)
                    tempObject['project_id'] = projectId;
                if (personId)
                    tempObject['person_id'] = personId;
                if (relationId)
                    tempObject['relation_id'] = relationId;
        ...
        dataType: 'json'//130226 měním text na json, snad to není issue
            ...
            .fail(function(error) {
                ...
                if (descriptionId) {
                    ...
                    $(currentTarget).fadeIn('slow');//hide info about server & reactivate currentTarget        
            })
            .done(function(msg) {
...
//ad case 3
$(document).one('pageshow', '#stakeholdermap', function() {//one brání opakovanému attach
    ...
    $('#quadrant1 a, #quadrant2 a, #quadrant3 a, #quadrant4 a').click(function(e) {
        e.stopPropagation(
...
function stayInWebAppNow(selector) {
    ...
    if (("standalone" in window.navigator) && window.navigator.standalone) {
        ...
        if ($(selector).attr("target") === undefined || $(selector).attr("target") === "" || $(selector).attr("target") === "_self") {
            ...
            if (true) {//for all links
                //prevent default behavior (opening safari)
                my_error_log('stayInWebApp for selector=' + selector + ' dest=' + dest, 4);
...
function gi(inputname, newvalue) {//getInputValueByName
