...
if (typeof apiUrlColoursave === "undefined")
  var apiUrlColoursave = relativePathToBackyardJs + "/" + "dummy.json";
...
$(document).ready(function () {
  ...
  $("a.coloursave, button.coloursave").bind("click", function (e) {
    //(e){
    ...
    if ($("#" + tempSelector).data("coloursave-context")) {
      ...
      if (IsJsonString(temp2)) {
        ...
        my_error_log(
          tempSelector +
            " has invalid context. JSON expected. Received:" +
            temp2,
          3,
        );
      }
  ...
  $('select.coloursave, input[type="radio"].coloursave').change(function () {
    ...
    if ($("[name=" + tempSelector + "]").is('input[type="radio"]')) {
      if (
        typeof $("[name=" + tempSelector + "]").checkboxradio === "function"
      ) {
    ...
    if ($("[name=" + tempSelector + "]").data("coloursave-context")) {
      temp2 = JSON.stringify(
        $("[name=" + tempSelector + "]").data("coloursave-context"),
      );
      //console.log(temp2);
      if (IsJsonString(temp2)) {
        tempContext = temp2;
  ...
  var jqxhr = $.ajax({
    ...
    .fail(function (error) {
      if (error.responseText) {
        ...
        my_error_log(
          "failed json reply: " + error.responseText,
          2,
          apiUrlErrorLog,
        );
